...
### F1 — Collector industrialization + canonical positions  ·  status: PLANNED
...
- [ ] **D6 — one canonical `src/data/positions.ts`** (allowlist + aliases + slot→position overrides + group taxonomy incl. ST), unit-tested; all 4 scattered tables + collector `normalize.ts` import it. *Pulled forward from F3 — de-risks F2/F3/F6.* (Fold in `normalize/positionMapping.ts`.)
...
- **Gate:** re-collect pilots byte-comparable ± vintage; tests green.
